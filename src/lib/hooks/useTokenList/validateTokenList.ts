import type { TokenInfo, TokenList } from '@ariswap/token-lists'
...
const validator = new Promise<Ajv>(async (resolve) => {
  const [ajv, schema] = await Promise.all([import('ajv'), import('@ariswap/token-lists/src/tokenlist.schema.json')])
